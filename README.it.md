...
📖 **Altre lingue:** [English](README.md) · [Español](README_es.md)
...
Se stai costruendo un'antenna filare multibanda, la domanda eterna è: *quanto deve essere lungo il filo?* Questo strumento risponde a questa domanda in modo empirico anziché a intuito.

Modella tre topologie, selezionabili con `--antenna-type`:

- **`long-wire`** (predefinito) — un radiatore end-fed o inclinato, con contrappeso, picchetto di terra o spezzone di coassiale opzionale come percorso di ritorno RF. Adattato con un **UnUn**.
- **`ocfd`** — un dipolo alimentato fuori centro (Windom): entrambi i bracci irradiano, il dipolo è il proprio percorso di ritorno, e `--wire-len` / `--cp-len` diventano il braccio lungo e quello corto. Adattato con un **balun**.
...
- 📐 **Tre topologie di antenna** — long wire end-fed (predefinito), dipolo alimentato fuori centro (Windom) e Carolina Windom, ciascuna con il proprio dispositivo di adattamento, modello empirico di riserva e metrica di qualità geometrica (`--antenna-type`).
...
- 🧰 **Calcolatori di adattamento integrati** — la scheda UnUn/Transmatch della GUI carica automaticamente le impedenze per banda dell'antenna vincente e progetta un **autotrasformatore UnUn** a banda larga (su un toroide di ferrite/polvere di ferro scelto da un database di 16 nuclei, oppure come solenoide in aria) o un **Transmatch a bobina con prese**, con spire, posizione delle prese, lunghezza del filo, stima del riscaldamento del nucleo e un disegno costruttivo salvabile come PNG.
- ⚙️ **Modalità parallela e di raffinamento** — `--fast-run` risolve in parallelo i file `nec2c` indipendenti (`--jobs N` fissa il numero di processi) e `--test-window` trasforma ogni tentativo in uno sweep più fine della regione migliore invece che in uno più ampio.
...
| `README_es.md`, `README_it.md`                                  | Traduzioni spagnola e italiana di questo README.                                                                                                                  |
...
> ⚠️ **Nota sui percorsi dei file:** la tabella qui sopra descrive la nuova organizzazione (`src/`, `documentation/`, `others/`). Finché questa riorganizzazione non arriva su `main`, il repository pubblicato usa ancora i vecchi nomi piatti — `nec2_length_optimizer.py`, `NEC2_Length_Optimizer_Manual_IT.md`, `README.es.md` / `README.it.md`, `NEC2_Length_Optimizer_Setup.exe`, `nec2-optimizer-x86_64.AppImage`, `nec2_optimizer_installer.nsi`, `build_appimage.sh` e `nec2c.exe`, tutti nella radice del repository. Se un percorso di questo documento restituisce 404, cercane il nome vecchio nella radice, e leggi `python src/Long_Wire_Antenna.py` in ogni esempio come `python nec2_length_optimizer.py`.
...
`colorama`, `matplotlib`, `numpy`, `reportlab` e `pillow` sono tutti facoltativi — lo script si adatta senza problemi (niente colori, niente grafico/PDF) se non sono installati. `numpy` serve solo per i diagrammi del pattern di radiazione e viene normalmente installato in automatico come dipendenza di `matplotlib`, quindi raramente occorre installarlo a parte. `pillow` (`PIL`) serve solo per incorporare i PNG di costruzione/radiazione nel brochure PDF; senza di esso il PDF viene comunque generato, con un segnaposto "non disponibile" nella sezione immagine interessata.
...
| `--feed-model {straddle,junction}`                                                                       | Dove sta la carta sorgente di NEC-2 rispetto alla giunzione radiatore/contrappeso. `straddle` (predefinito) converge molto più in fretta; `junction` riproduce risultati vecchi ed è forzato per `carolina-windom`. |
| `--antenna-type {long-wire,ocfd,carolina-windom}`                                                        | Topologia dell'antenna. `long-wire` (predefinito) è il radiatore end-fed descritto sopra. I tipi dipolo trasformano `--wire-len` / `--cp-len` nei **bracci lungo e corto** di un dipolo alimentato fuori centro, rifiutano `--no-counterpoise` e progettano un **balun** invece di un UnUn. `carolina-windom` aggiunge una sezione verticale radiante e forza `--feed-model junction`. |
| `--total-len`, `--offset`, `--offset-min/max/step`                                                       | Tipi alimentati fuori centro: lunghezza totale di entrambi i bracci e frazione del braccio corto (predefinito `0.3333`, il classico terzo del Windom; intervallo valido `0.10`–`0.49`) — il modo abituale di specificare un Windom. Il trio `-min/max/step` delimita la scansione dell'offset. |
| `--balun-ratio`, `--balun-kind`, `--balun-core`, `--balun-turns`                                         | Progetto del balun per i tipi dipolo: rapporto (`auto`, oppure uno fra 2/4/6/9 — un balun a linea di trasmissione esiste a quei rapporti e non nei valori intermedi), topologia (balun di corrente `guanella` come predefinito, o `ruthroff`), nucleo toroidale e spire per linea. |
| `--cw-vert-len`, `--cw-isolator-z`                                                                       | Solo Carolina Windom: lunghezza della sezione verticale radiante (predefinito 3.0 m) e isolatore di linea modellato come impedenza serie finita (es. `1000,2000`) invece che come aperto ideale — utile per studiare quanto costa un choke insufficiente. |
| `--feed-choke`, `--match-model {ideal,real}`                                                             | Progetta anche un choke di modo comune per la linea di alimentazione (sempre progettato per `carolina-windom`, dove *è* l'isolatore di linea); e se il ROS attraverso il dispositivo di adattamento è idealizzato o include la reattanza magnetizzante finita del balun. |
| `--fast-run`, `--jobs N`                                                                                 | `--fast-run` è la *politica* di accelerazione (sweep grossolano, liste di raffinamento/ri-classificazione più corte, un solo braccio di convergenza) — una risposta più economica. `--jobs N` risolve in parallelo i deck `nec2c` indipendenti ed è onorato **con o senza** `--fast-run`: accorcia solo il tempo di calcolo, non cambia mai un numero pubblicato. Predefiniti: 1 (seriale), oppure uno per core con tetto di 16 sotto `--fast-run`. Le impedenze pubblicate restano alla densità fine. |
| `--retry N`, `--test-window`, `--refine-top N`                                                           | Riesegue lo sweep fino a N volte in più: spostando la finestra verso l'esterno per impostazione predefinita, oppure — con `--test-window` — affinandola attorno ai migliori `--refine-top` candidati con metà del passo di griglia. |
| `--converge`                                                                                             | Risimula il vincitore a 0.5× e 2× la segmentazione di lavoro e riporta quanto si muovono ancora R e X. Solo in modalità NEC2.                                                                            |
...
La scheda UnUn/Transmatch della GUI scrive altri file, su richiesta e non automaticamente: `unun_toroid.png` o `unun_solenoid.png` (il disegno costruttivo dell'UnUn), `transmatch_coil.png` (il disegno della bobina con prese) e `unun_transmatch.txt` (un'esportazione dei risultati di entrambi i calcolatori, salvata accanto a una copia `_transmatch.png` del disegno della bobina).
