...
| `--feed-model {straddle,junction}`                                                                       | Dove sta la carta sorgente di NEC-2 rispetto alla giunzione radiatore/contrappeso. `straddle` (predefinito) converge molto più in fretta; `junction` riproduce risultati vecchi.                          |
| `--fast-run`, `--jobs N`                                                                                 | Politica di accelerazione (soluzioni `nec2c` concorrenti, sweep grossolano, liste di raffinamento/ri-classificazione più corte). `--jobs` è **ignorato senza `--fast-run`**. Le impedenze pubblicate restano alla densità fine. |
