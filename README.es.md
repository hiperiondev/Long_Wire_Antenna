...
| `--feed-model {straddle,junction}`                                                                       | Dónde se ubica la tarjeta fuente de NEC-2 respecto de la unión radiador/contrapeso. `straddle` (por defecto) converge mucho más rápido; `junction` reproduce resultados antiguos.                        |
| `--fast-run`, `--jobs N`                                                                                 | Política de aceleración (resolución concurrente de `nec2c`, barrido grueso, listas de refinamiento/reclasificación más cortas). `--jobs` **se ignora sin `--fast-run`**. Las impedancias publicadas siguen calculándose a densidad fina. |
