...
📖 **Otros idiomas:** [English](README.md) · [Italiano](README_it.md)
...
Si estás armando una antena de hilo multibanda, la eterna pregunta es: *¿qué largo tiene que tener el hilo?* Esta herramienta responde esa pregunta de forma empírica en lugar de a ojo.

Modela tres topologías, elegidas con `--antenna-type`:

- **`long-wire`** (por defecto) — un radiador alimentado por el extremo o inclinado, con contrapeso, jabalina de tierra o stub de coaxial opcional como camino de retorno de RF. Se adapta con un **UnUn**.
- **`ocfd`** — un dipolo alimentado fuera del centro (Windom): ambos brazos radian, el dipolo es su propio camino de retorno, y `--wire-len` / `--cp-len` pasan a ser el brazo largo y el corto. Se adapta con un **balun**.
...
- 📐 **Tres topologías de antena** — long wire alimentado por el extremo (por defecto), dipolo alimentado fuera del centro (Windom) y Carolina Windom, cada una con su propio dispositivo de adaptación, modelo empírico de respaldo y métrica de calidad geométrica (`--antenna-type`).
...
- 🧰 **Calculadoras de adaptación integradas** — la pestaña UnUn/Transmatch de la GUI carga automáticamente las impedancias por banda de la antena ganadora y diseña un **autotransformador UnUn** de banda ancha (sobre un toroide de ferrita/polvo de hierro de una base de 16 núcleos, o como solenoide con núcleo de aire) o un **Transmatch de bobina con derivaciones**, con vueltas, posición de las derivaciones, longitud de hilo, estimación de calentamiento del núcleo y un dibujo constructivo que puede guardarse como PNG.
- ⚙️ **Modos paralelo y de refinamiento** — `--fast-run` resuelve decks `nec2c` independientes en paralelo (`--jobs N` fija la cantidad de procesos) y `--test-window` convierte cada reintento en un barrido más fino de la mejor región en lugar de uno más amplio.
...
| `README_es.md`, `README_it.md`                                 | Traducciones al español y al italiano de este README.                                                                                                                           |
...
> ⚠️ **Nota sobre las rutas de archivo:** la tabla de arriba describe la organización nueva (`src/`, `documentation/`, `others/`). Hasta que esa reorganización llegue a `main`, el repositorio publicado sigue usando los nombres planos antiguos — `nec2_length_optimizer.py`, `NEC2_Length_Optimizer_Manual_ES.md`, `README.es.md` / `README.it.md`, `NEC2_Length_Optimizer_Setup.exe`, `nec2-optimizer-x86_64.AppImage`, `nec2_optimizer_installer.nsi`, `build_appimage.sh` y `nec2c.exe`, todos en la raíz del repositorio. Si una ruta de este documento da 404, buscá su nombre antiguo en la raíz, y leé `python src/Long_Wire_Antenna.py` en cada ejemplo como `python nec2_length_optimizer.py`.
...
>
> ⚠️ **Bug conocido (Windows, solo si evitás `run_gui.bat`):** el paso 3 copia el motor a `C:\Program Files\OpenNEC\nec2c.exe` y a `C:\Program Files (x86)\OpenNEC\nec2c.exe`. Sin embargo, las rutas de respaldo fijas de `Long_Wire_Antenna.py` para Windows buscan un archivo llamado **`onec.exe`** en esas mismas dos carpetas, no `nec2c.exe` — los nombres de archivo no coinciden, así que la búsqueda de respaldo integrada en `Program Files\OpenNEC` **no** va a encontrar la copia que dejó el instalador ahí. Esto no afecta el flujo normal del acceso directo del Escritorio, porque `run_gui.bat` configura la variable de entorno `NEC2C` directamente y nunca depende de este respaldo. Solo importa si abrís una terminal normal después de instalar y ejecutás `python Long_Wire_Antenna.py` (o `py Long_Wire_Antenna.py`) vos mismo, *sin* pasar por `run_gui.bat`, sin tener `%NEC2C%` configurada, y sin `%INSTDIR%\nec2c` en tu `PATH` — en ese caso puntual, el modo NEC2 va a reportar que no encuentra el binario aunque haya un motor funcional en `Program Files\OpenNEC`. Se puede evitar con `--nec2c "C:\Program Files\OpenNEC\nec2c.exe"`, configurando `NEC2C` vos mismo, o usando el acceso directo del Escritorio / `run_gui.bat`.
...
| `--feed-model {straddle,junction}`                                                                       | Dónde se ubica la tarjeta fuente de NEC-2 respecto de la unión radiador/contrapeso. `straddle` (por defecto) converge mucho más rápido; `junction` reproduce resultados antiguos, y se fuerza para `carolina-windom`. |
| `--antenna-type {long-wire,ocfd,carolina-windom}`                                                        | Topología de la antena. `long-wire` (por defecto) es el radiador alimentado por el extremo descrito arriba. Los tipos dipolo convierten `--wire-len` / `--cp-len` en los **brazos largo y corto** de un dipolo alimentado fuera del centro, rechazan `--no-counterpoise` y diseñan un **balun** en lugar de un UnUn. `carolina-windom` agrega una sección vertical radiante y fuerza `--feed-model junction`. |
| `--total-len`, `--offset`, `--offset-min/max/step`                                                       | Tipos alimentados fuera del centro: longitud total de ambos brazos y fracción del brazo corto (por defecto `0.3333`, el tercio clásico del Windom; rango válido `0.10`–`0.49`) — la forma habitual de especificar un Windom. El trío `-min/max/step` acota el barrido del offset. |
| `--balun-ratio`, `--balun-kind`, `--balun-core`, `--balun-turns`                                         | Diseño del balun para los tipos dipolo: relación (`auto`, o una de 2/4/6/9 — un balun de línea de transmisión existe en esas relaciones y no en las intermedias), topología (balun de corriente `guanella` por defecto, o `ruthroff`), núcleo toroidal y vueltas por línea. |
| `--cw-vert-len`, `--cw-isolator-z`                                                                       | Solo Carolina Windom: longitud de la sección vertical radiante (por defecto 3.0 m), y el aislador de línea modelado como impedancia serie finita (p. ej. `1000,2000`) en lugar de un abierto ideal — útil para estudiar cuánto cuesta un choke insuficiente. |
| `--feed-choke`, `--match-model {ideal,real}`                                                             | Diseñar además un choke de modo común para la línea de alimentación (siempre se diseña para `carolina-windom`, donde *es* el aislador de línea); y si el ROE a través del dispositivo de adaptación es idealizado o incluye la reactancia magnetizante finita del balun. |
| `--fast-run`, `--jobs N`                                                                                 | `--fast-run` es la *política* de aceleración (barrido grueso, listas de refinamiento/reclasificación más cortas, un solo brazo de convergencia) — una respuesta más barata. `--jobs N` resuelve decks `nec2c` independientes en paralelo y se respeta **con o sin** `--fast-run`: solo acorta el tiempo de reloj, nunca cambia un número publicado. Por defecto: 1 (serial), o uno por núcleo con tope de 16 bajo `--fast-run`. Las impedancias publicadas siguen calculándose a densidad fina. |
| `--retry N`, `--test-window`, `--refine-top N`                                                           | Vuelve a ejecutar el barrido hasta N veces más: desplazando la ventana hacia afuera por defecto, o — con `--test-window` — afinándola alrededor de los mejores `--refine-top` candidatos con la mitad del paso de grilla. |
| `--converge`                                                                                             | Resimula el ganador a 0.5× y 2× la segmentación de trabajo y reporta cuánto se mueven todavía R y X. Solo en modo NEC2.                                                                                  |
...
La pestaña UnUn/Transmatch de la GUI escribe algunos archivos más, a pedido y no automáticamente: `unun_toroid.png` o `unun_solenoid.png` (el dibujo constructivo del UnUn), `transmatch_coil.png` (el dibujo de la bobina con derivaciones) y `unun_transmatch.txt` (una exportación de los resultados de ambas calculadoras, guardada junto a una copia `_transmatch.png` del dibujo de la bobina).
...
|---|---|---|
| `--mode nec2` termina de inmediato con un error de "binario no encontrado" | No se pudo localizar `nec2c` por ningún método de búsqueda, y `--mode nec2` no tiene alternativa automática | Instalá `nec2c`, o pasá `--nec2c /ruta/completa/a/nec2c`, o configurá la variable de entorno `$NEC2C`. Con `--mode auto`, en cambio, el script pasa automáticamente a `--mode empirical` en lugar de terminar. |
| En Windows, no se encuentra `nec2c`/`onec.exe` aunque el instalador funcionó bien, y **no** lo iniciaste con el acceso directo del Escritorio | Ejecutaste `Long_Wire_Antenna.py` directamente (sin pasar por `run_gui.bat`) sin tener `%NEC2C%` configurada — ver el [bug conocido más arriba](#opción-a--windows-la-más-fácil) sobre el desajuste de nombres `nec2c.exe`/`onec.exe` en `Program Files\OpenNEC` | Usá el acceso directo del Escritorio / `run_gui.bat`, o pasá `--nec2c "C:\Program Files\OpenNEC\nec2c.exe"` explícitamente, o configurá `NEC2C` vos mismo. |
| Una banda queda marcada como mal acoplada con todas las relaciones estándar de UnUn | La impedancia cruda de la antena en esa banda está lo bastante lejos de cada relación del barrido incluido (1:1, 1.5:1, 2:1, 3:1, 4:1, 6:1, 9:1, ...) como para que ninguna acerque el ROE a 1:1 | Esto es esperable en algunos diseños multibanda — una única estrategia de línea de alimentación no siempre puede acoplar bien bandas sin relación entre sí. Opciones: aceptar el ROE más alto en esa banda, usar la calculadora de Transmatch (bobina con tomas) en lugar de, o además de, el UnUn para esa banda, o volver a correr el optimizador con una ventana de búsqueda distinta/más angosta o con otra selección de `--active-bands` para encontrar una geometría que sacrifique menos en esa banda. |
| Falta `matplotlib`/`numpy`/`reportlab` pero se pidió un gráfico o PDF | Paquete opcional no instalado | La corrida igual se completa y escribe las salidas no gráficas (reporte de texto, CSV, archivo `.nec`); el script indica qué salida opcional se omitió y por qué. Instalá el/los paquete(s) faltante(s) con `pip install matplotlib numpy reportlab` (ver la [Opción C](#opción-c--ejecutar-el-script-python-directamente-windowsmacoslinux)) y volvé a correrlo. |

<!-- TODO(mantenedor): Las filas de arriba se reconstruyeron leyendo el flujo de control del
     propio script (ver find_nec2c(), el bloque de resolución de --mode en main(), el barrido
     de relaciones de UnUn en STANDARD_UNUN_RATIOS, y las protecciones de importación opcional
     al principio del archivo), no a partir de mensajes de error capturados textualmente.
     Considerar reemplazar las descripciones libres por los mensajes de consola literales (las
     claves T("...")) la próxima vez que se actualice este documento, y agregar los casos de
     convergencia/--retry de NEC-2 si aparecen en la práctica. -->
