...
📖 **Other languages:** [Español](README_es.md) · [Italiano](README_it.md)
...
- 🧰 **Matching-network designers built in** — the GUI's UnUn/Transmatch tab loads the winning antenna's per-band impedances automatically and designs either a wideband **UnUn autotransformer** (on a ferrite/iron-powder toroid from a 16-core database, or as an air-core solenoid) or a **tapped-coil Transmatch**, complete with turns, tap positions, wire lengths, core-heating estimates and a construction drawing you can save as a PNG.
- ⚙️ **Parallel and refinement modes** — `--fast-run` solves independent `nec2c` decks concurrently (`--jobs N` to set the worker count), and `--test-window` turns each retry into a finer sweep of the best region instead of a wider one.
...
| `README_es.md`, `README_it.md`                                 | Spanish and Italian translations of this README.                                                                                                            |
...
The GUI's UnUn/Transmatch tab writes a few more files, on demand rather than automatically: `unun_toroid.png` or `unun_solenoid.png` (the UnUn construction drawing), `transmatch_coil.png` (the tapped-coil drawing), and `unun_transmatch.txt` (an Export of both calculators' results, saved alongside a `_transmatch.png` copy of the coil drawing).
